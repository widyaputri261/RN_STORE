...
<form action="<?= base_url(); ?>snap/finish?invoice=<?= $ord['invoice_code']; ?>" id="payment-form" method="post">
...
<div class="wrapper">
    ...
    <div class="core">
        <?php if ($ord['courier'] == "cod") { ?>
            <?php if ($ord['status'] != "4") { ?>
                <h2 class="title">COD (Cash On Delivery)</h2>
                <p>Pesanan ini menggunakan metode COD (Cash On Delivery). Silakan menghubungi penjual melalui WhatsApp dengan cara <a href="https://wa.me/<?= $this->Settings_model->general()["whatsappv2"]; ?>?text=Halo kak, saya membeli produk di <?= $this->Settings_model->general()["app_name"]; ?> menggunakan metode COD/Cash On Delivery dengan Order ID <?= $ord['invoice_code']; ?>" target="_blank" class="btn btn-success btn-sm">klik disini</a></h2>
                    <hr>
                    <hr>
                <?php } ?>
            <?php } ?>
            <?php if ($ord['status'] == 3) { ?>
                <div class="alert alert-info">Jika pesanan telah sampai tujuan, silahkan tekan tombol dibawah</div>
